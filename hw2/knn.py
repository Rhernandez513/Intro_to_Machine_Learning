...
class KNN(BinaryClassifier):
    ...
    def predict(self, X):
        ...
        Our return value should be the sign of the 'vote' in favor of a positive
        or negative label.  In particular, if, in our neighbor set,
        there are 5 positive training examples and 2 negative
        examples, we return sign(5-2)=sign(3)=1.

        If, in our neighbor set, there are 3 positive
        training examples and 5 negative examples,
        we return sign(3-5)=sign(-2)=-1
        ...
        if self.trY.size == 0:
            ...
            dist = [] # distances
            for x in self.trX: # neighbors
                dist.append(norm(X - x)) # shuffled distances

            _dist = argsort(dist) # sorted distances

            for i in range(K):
                idx = _dist[i]
                val += sign(self.trY[idx])

            return sign(val)
        else:
            # this is an epsilon ball model (which you will not be implementing!)
